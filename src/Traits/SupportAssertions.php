...
namespace dbizapps\Headless\Traits;

use Facebook\WebDriver\Exception\NoSuchElementException;
use Facebook\WebDriver\Remote\RemoteWebElement;
use PHPUnit\Framework\Assert as PHPUnit;

trait SupportAssertions
{
    ...
    public function assertTitle($title)
    {
        PHPUnit::assertEquals(
            ...
            "Expected title [{$title}] does not equal actual title [{$this->browser->getTitle()}]."
        );
    ...
    {
        PHPUnit::assertTrue(
            strpos($this->browser->getTitle(), $title) !== false, 
            "Did not see expected text [{$title}] within title [{$this->browser->getTitle()}]."
        );
     ...
     * @return $this
     */
    public function assertSeeIn($selector, $compare)
    {
        $text = $this->browser->find($selector)->getText();
 
        PHPUnit::assertTrue(
            strpos($text, $compare) !== false, 
            "Did not see expected text [{$text}] within element [{$selector}]."
        );
    ...
    public function assertDontSeeIn($selector, $text)
    {
        ...
        PHPUnit::assertFalse(
            strpos($text, $compare) == false, 
            "Saw unexpected text [{$text}] within element [{$selector}]."
        );
    ...
    {
        expression = $prefix.preg_replace('/^(?:return )+/u', '', $expression);

        PHPUnit::assertEquals(
            $expected,
            $this->browser->executeScript($expression),
...
JS;
    ...
    {
        $actualMessage = $this->browser->switchTo()->alert()->getText();
    ...
    public function assertFocused($field)
    {
        ...
        PHPUnit::assertTrue(
            $this->browser->switchTo()->activeElement()->equals($element),
